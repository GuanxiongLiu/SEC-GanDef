...
def make_gandef_model(name, nb_classes=10, input_shape=(None, 32, 32, 3)):
    clf_layers = [Dropout(rate=0.2, 
                          name="clf_dropout_1"),
                  Conv2D(output_channels=96, 
                         kernel_shape=(3, 3), 
                         strides=(1, 1), 
                         padding="SAME", 
                         name="clf_conv2d_1", 
                  ...
                  Conv2D(output_channels=10, 
                         ...
                         kernel_initializer=tf.keras.initializers.he_normal()),
                  ReLU(name="clf_act_9"),
                  GlobalAvgPool2D(name="clf_globalavgpool2d_1"),
...
def train_gandef_model(train_start=0, train_end=60000, test_start=0, 
    ...
    gandef_model = make_gandef_model(name="model_gandef")
    preds_clean = gandef_model(aug_x_1)
    adv_params = {
        'eps': float(8/255)*2.,
    ...
    adv_att = MadryEtAl(gandef_model, sess=sess)
    adv = tf.stop_gradient(adv_att.generate(x, **adv_params))
    preds_adv = gandef_model(adv)
    ...
    gan_train_v2(sess, x, y_soft, preds_clean, X_train, Y_train, 
                 ...
                 predictions_adv=preds_adv, evaluate=None, args=train_params, rng=rng, 
                 var_list=gandef_model.get_gan_params())
    
    # Evaluate the accuracy of the MNIST model on Clean examples
    preds_clean = gandef_model(x)
    eval_params = {
        'batch_size': 128,
        'use_dic': False,
        'is_clean': True,
    ...
    fgsm_att = FastGradientMethod(gandef_model, sess=sess)
    fgsm_adv = fgsm_att.generate(x, **fgsm_params)
    preds_fgsm_adv = gandef_model(fgsm_adv)
    eval_params = {
        'batch_size': 128,
        'use_dic': False,
        'is_clean': False,
    ...
    bim_att = BasicIterativeMethod(gandef_model, sess=sess)
    bim_adv = bim_att.generate(x, **bim_params)
    preds_bim_adv = gandef_model(bim_adv)
    eval_params = {
        'batch_size': 128,
        'use_dic': False,
        'is_clean': False,
    ...
    pgd_att = MadryEtAl(gandef_model, sess=sess)
    pgd_adv = pgd_att.generate(x, **pgd_params)
    preds_pgd_adv = gandef_model(pgd_adv)
    eval_params = {
        'batch_size': 128,
        'use_dic': False,
        'is_clean': False,
...
def main(argv=None):
    train_gandef_model(smoke_test=FLAGS.smoke_test, 
                                      save=FLAGS.save, 
